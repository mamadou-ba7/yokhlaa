...
import { calculatePassengerCancellationFee, canPassengerCancel, getCancellationMessage } from '../../lib/cancellation';
import { requestMatch } from '../../lib/matching';
...
const RIDE_CLASSES = [
  { id: 'eco', name: 'Eco', icon: 'car-outline', mult: 0.9, desc: 'Economique' },
  { id: 'confort', name: 'Confort', icon: 'car-sport-outline', mult: 1.1, desc: 'Vehicule confortable' },
];
...
export default function PassengerMapScreen({ navigation }) {
  ...
  // ── Address search (Nominatim + local places) ─────────────
  const onSearch = (txt) => {
    setSearch(txt);
    ...
    // Recherche locale instantanée dans les lieux populaires
    const normalize = (s) => s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    const q = normalize(txt);
    const localResults = POPULAR_PLACES.filter(p =>
      normalize(p.name).includes(q) || normalize(p.area).includes(q)
    ).slice(0, 5).map(p => ({ ...p, source: 'local' }));
    setResults(localResults);

    // Google Places Autocomplete — trouve rues, commerces, lieux, tout Dakar
    timer.current = setTimeout(async () => {
      setSearching(true);
      try {
        const GOOGLE_KEY = 'AIzaSyDTK9VG9QU_b5hSKVXf4bNQG1cnv1ZHaQM';
        const input = encodeURIComponent(txt);
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 8000);

        // Places Autocomplete — retourne tous les types de lieux
        const res = await fetch(
          `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${input}&key=${GOOGLE_KEY}&language=fr&components=country:sn&location=14.7167%2C-17.4677&radius=50000&types=establishment|geocode`,
        ...
        if (data.predictions?.length) {
          const apiResults = await Promise.all(
            data.predictions.slice(0, 8).map(async (p) => {
              // Place Details pour obtenir les coordonnees
              try {
                const detRes = await fetch(
                  `https://maps.googleapis.com/maps/api/place/details/json?place_id=${p.place_id}&key=${GOOGLE_KEY}&fields=geometry,name,formatted_address`
                );
                const det = await detRes.json();
                if (det.result?.geometry?.location) {
                  ...
                  return {
                    name: mainText,
                    area: secondaryText,
                    lat: det.result.geometry.location.lat,
                    lng: det.result.geometry.location.lng,
                    source: 'google',
                    place_id: p.place_id,
                  };
                }
          ...
          const validResults = apiResults.filter(Boolean);
          const seen = new Set(localResults.map(r => `${r.lat.toFixed(3)},${r.lng.toFixed(3)}`));
        ...
        if (e.name !== 'AbortError') {
          ...
          if (localResults.length === 0) setSearchError(true);
        }
      }
      setSearching(false);
    }, 300);
  ...
  const requestRide = async () => {
    ...
    try {
      ...
      // Lancer le matching intelligent (remplace le broadcast global)
      try {
        await requestMatch(data.id);
      } catch (matchErr) {
        console.warn('Matching error:', matchErr);
        // Fallback : notification broadcast historique (pas bloquant)
  ...
  const cancel = async () => {
    ...
    if (!canPassengerCancel(ride)) {
      const msg = getCancellationMessage(-1);
      Alert.alert(msg.title, msg.message);
      return;
    }

    // Calculer les frais
    const fee = calculatePassengerCancellationFee(ride);
    const msg = getCancellationMessage(fee);
    ...
    Alert.alert(
      ...
      [
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            // Partager le lien de suivi en temps reel
            if (ride?.share_token) {
              try {
                await Share.share({
                  message: `URGENCE — Je suis en course Yokh Laa. Suivez ma position en temps reel : https://yokhlaa.app/track/${ride.share_token}`,
                });
              } catch (e) {
                console.warn('Share error:', e);
              }
            ...
            // Appeler la police
            Linking.openURL(`tel:${SOS_NUMBER}`);
          },
        },
      ]
    );
          ...
          <ScrollView style={s.slist} keyboardShouldPersistTaps="handled">
            ...
            {showHistoryAndPopular && (
              <>
                ...
                {POPULAR_PLACES.slice(0, 12).map((p, i) => (
    ...
    <View style={s.root}>
      ...
      <View style={s.top}>
        <TouchableOpacity
          ...
          onPress={() => {
            tapLight();
            (navigation.getParent() || navigation).navigate('Profil');
          }}
        >
        ...
        <View style={s.sheet}>
          ...
          <View style={s.classRow}>
            {RIDE_CLASSES.map(c => {
              ...
              const etaForClass = c.id === 'eco' ? eta + 1 : eta;
              return (
                <TouchableOpacity key={c.id} style={[s.classCardH, active && s.classCardActiveH]} onPress={() => setRideClass(c.id)} activeOpacity={0.7}>
                  <Ionicons name={c.icon} size={26} color={active ? COLORS.green : COLORS.dim} />
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    <Text style={[s.classNameH, active && { color: COLORS.white }]}>{c.name}</Text>
                    <Text style={s.classDescH}>{c.desc} · {etaForClass} min</Text>
                  </View>
                  <Text style={[s.classPriceH, active && { color: COLORS.green }]}>{getPrice(c.id).toLocaleString()} F</Text>
                </TouchableOpacity>
              );
            })}
          </View>
          ...
          {/* Paiement : directement au chauffeur (cash / wave / OM) — pas gere par l'app */}
          <View style={s.payInfo}>
            <Ionicons name="information-circle-outline" size={14} color={COLORS.dim} />
            <Text style={s.payInfoTxt}>Paiement directement au chauffeur (especes, Wave ou Orange Money)</Text>
          </View>
...
const s = StyleSheet.create({
  ...
  // Ride classes horizontal cards
  classRow: { gap: 10, marginBottom: 14 },
  classCardH: {
  ...
  classCardActiveH: { borderColor: COLORS.green, backgroundColor: 'rgba(34,197,94,0.06)' },
  classNameH: { fontSize: 16, fontWeight: '700', color: COLORS.dim, marginBottom: 2 },
  classDescH: { fontSize: 12, color: COLORS.dim2 },
  classPriceH: { fontSize: 16, fontWeight: '800', color: COLORS.dim },
